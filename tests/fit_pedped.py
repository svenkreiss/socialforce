...
def visualize(file_prefix, V, initial_parameters, fit_result):
    b = np.linspace(0, 3)
    ...
    V.set_parameters(torch.tensor(fit_result.x))
...
def test_opposing():
    ...
    res = scipy.optimize.minimize(f, parameters, method='L-BFGS-B', options={'eps': 1e-4})
    print(res)
...
def test_opposing_mlp():
    ...
    visualize('docs/mlp_', V, initial_parameters, res)
